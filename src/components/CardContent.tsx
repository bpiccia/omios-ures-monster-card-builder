...
export function CardContent({
	...
	const hasSpecialAttacks =
		monster.hasSpecialAttacks && monster.specialAttacks.length > 0
		...
		>
			...
			>
				...
				<div className="linebreak-container linebreak-small">
					<img
						src={getAssetUrl("/linebreak.png")}
						alt=""
						className="linebreak-image"
					/>
				</div>
							...
							{monster.abilities
								...
								.map((ability) => (
									<div key={ability.id} className="ability-item">
										...
										{ability.desc && (
											<span className="ability-desc">{ability.desc}</span>
										)}
									</div>
								))}
						</div>
				...
				{hasSpecialAttacks && (
					<>
						...
						<h3 className="section-header">
							{dict.cardHeaders.specialAttacks}
						</h3>
						<div className="section-content">
							{monster.specialAttacks
								.filter((specialAttack) => specialAttack.name)
								.map((specialAttack) => (
									<div key={specialAttack.id} className="ability-item">
										<span className="ability-name">{specialAttack.name}.</span>
										{specialAttack.desc && (
											<span className="ability-desc">{specialAttack.desc}</span>
										)}
									</div>
								))}
						</div>
							...
							{monster.spells
								...
								.map((spell) => (
									<div key={spell.id} className="ability-item">
										...
										{spell.desc && (
											<span className="ability-desc">{spell.desc}</span>
										)}
									</div>
								))}
						</div>
